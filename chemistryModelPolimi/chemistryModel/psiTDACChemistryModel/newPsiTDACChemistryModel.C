...
Foam::autoPtr<Foam::psiTDACChemistryModel> Foam::psiTDACChemistryModel::New
(
    const fvMesh& mesh
)
{
    word psiTDACChemistryModelType;
    word thermoTypeName;
    word userModel;
    ...
    // dictionary is entered in the database twice
    {
        ...
        chemistryPropertiesDict.lookup("psiTDACChemistryModel") >> userModel;
        ...
        psiTDACChemistryModelType =
            className + '<' + typeName + ',' + thermoTypeName + '>';
    }
    if (debug)
    {
        Info<< "Selecting psiTDACChemistryModel " << psiTDACChemistryModelType << endl;
    }
    else
    {
        Info<< "Selecting psiTDACChemistryModel " << userModel << endl;
    }

    fvMeshConstructorTable::iterator cstrIter =
        fvMeshConstructorTablePtr_->find(psiTDACChemistryModelType);

    if (cstrIter == fvMeshConstructorTablePtr_->end())
    {
        if (debug)
        {
            FatalErrorIn("psiTDACChemistryModelBase::New(const mesh&)")
                << "Unknown psiTDACChemistryModel type " << psiTDACChemistryModelType
                << nl << nl << "Valid psiTDACChemistryModel types are:" << nl
        ...
        else
        {
            ...
            FatalErrorIn("psiTDACChemistryModelBase::New(const mesh&)")
                << "Unknown psiTDACChemistryModel type " << userModel
                << nl << nl << "Valid psiTDACChemistryModel types are:" << nl
                << models << nl << exit(FatalError);
        }
    }

    return autoPtr<psiTDACChemistryModel>
